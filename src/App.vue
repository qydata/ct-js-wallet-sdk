...
export default {
  ...
  async mounted() {
    window.ctApp = this
    // window.ctApp.sendAddressTrans = this.sendAddressTrans()
    // window.ctApp.sendContractTrans = this.sendContractTrans()
    // window.ctApp.connectWallet = this.connectWallet()
    // 使用暗色模式
  ...
  methods: {
    async sendAddressTrans(from, to, value) {

      this.walletAddress = from
      this.addressForm.to = to
      this.addressForm.value = value
      return await this.postMessage(1)
    },
    async sendContractTrans(from, to, contractAddress, value, abi, method, params) {
      this.walletAddress = from
      this.addressForm.to = to
      this.addressForm.contractAddress = contractAddress
      this.addressForm.value = value
      this.addressForm.abi = abi
      this.addressForm.method = method
      this.addressForm.params = params
      return  await this.postMessage(2)
    },
    connectWallet() {
      ...
      return new Promise((resolve, reject) => {
        window.addEventListener('message', function (e) {
          ...
          if (event.data && event.data.type == 'connectWallet') {
            // 交易体封装
    ...
    async postMessage(type) {
      ...
      return new Promise((resolve, reject) => {
        window.addEventListener('message', function (e) {
          ...
          if (event.data && event.data.type == 'trans_result_address') {
            // 交易体封装
            // 交易体封装
            console.log('交易结果', event.data)
            that.addTx = event.data
          ...
          if (event.data && event.data.type == 'trans_result_contract') {
            // 交易体封装
            // 交易体封装
            console.log('交易结果', event.data)
            that.addTx = event.data
